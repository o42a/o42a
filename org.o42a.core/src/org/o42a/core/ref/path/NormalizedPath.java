...
final class NormalizedPath implements NormalPath {
	...
	public void writeCond(CodeDirs dirs, HostOp host) {
		if (this.inline != null) {
			...
			this.path.op(host).value().writeCond(dirs);
		}
	}
	...
	public ValOp writeValue(ValDirs dirs, HostOp host) {
		...
		return this.path.op(host).value().writeValue(dirs);
	}

	public final NormalPath done(Analyzer analyzer, boolean done) {

		// Inform the doubt that the path is normalized.
		final UserInfo user = this.path.doubt(analyzer).pathNormalized();
	...
	private NormalPath build(UserInfo user) {
