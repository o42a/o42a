...
public abstract class AbstractObjFld<
		F extends RefFld.Op<F>,
		T extends RefFld.Type<F>> extends RefFld<
				F,
				T,
				Ptr<ObjFldConf.Op>,
	...
	@Override
	protected ObjVmtRecord<F, T> createVmtRecord() {
		return new ObjVmtRecord<>(this);
	}

	@Override
	protected ObjFldOp<F, T> op(
			Code code,
			ObjOp host,
			OpMeans<F> ptr) {
	...
	protected abstract
	FunctionBuilder<ObjectConstructorFn> constructorBuilder();
	...
	private ObjFldConf reuseConfFrom(Obj ascendant) {
		...
		final AbstractObjFld<?, ?> src =
				(AbstractObjFld<?, ?>) ascendantIR.bodies().findFld(getKey());
