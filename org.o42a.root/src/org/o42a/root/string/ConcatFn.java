...
public class ConcatFn extends Fn<ConcatFn> {

	public static final ExtSignature<Void, ConcatFn> CONCAT =
			customSignature("ConcatF", 3)
			.addPtr("output", VAL_TYPE)
			.addPtr("what", VAL_TYPE)
			.addPtr("with", VAL_TYPE)
			.returnVoid(c -> new ConcatFn(c));

	private ConcatFn(FuncCaller<ConcatFn> caller) {
		super(caller);
	}
