...
public class EllipsisTest extends GrammarTestCase {
	...
	public void onlyEllipsis() {
		...
		final EllipsisNode ellipsis = singleStatement(EllipsisNode.class, sentence);

		assertNull(sentence.getMark());
		assertRange(0, 3, ellipsis);
		assertNull(ellipsis.getTarget());
		assertThat(ellipsis.getMark().getType(), is(EllipsisNode.Mark.ELLIPSIS));
	}
	...
	public void untargetedEllipsis() {
		...
		final MemberRefNode statement = statement(MemberRefNode.class, sentence, 0, 2);
		final EllipsisNode ellipsis = statement(EllipsisNode.class, sentence, 1, 2);
