...
final class DebugDoFn extends Fn<DebugDoFn> {

	static final ExtSignature<Void, DebugDoFn> DEBUG_DO =
			customSignature(DEBUG_ID.sub("DoF"), 2)
			.addPtr("stack_frame", DEBUG_STACK_FRAME_TYPE)
			.addPtr("comment")
			.returnVoid(c -> new DebugDoFn(c));

	private DebugDoFn(FuncCaller<DebugDoFn> caller) {
		super(caller);
	}
